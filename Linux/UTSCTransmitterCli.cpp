// UTSCTransmitter.cpp : Defines the entry point for the application.
//
...
#define TX_BUFFER_SIZE 40 * 1024 * 16
...
static scmplx  m_samples[2000000];
...
void *rrcThread(void *threadInfoArg) {
	rrcThreadInfo *threadInfo = (rrcThreadInfo*)threadInfoArg;
	scmplx *sin = threadInfo->sin;
	scmplx *sout = threadInfo->sout;
	unsigned int startI = threadInfo->startI;
	unsigned int endI = threadInfo->endI;

	int32_t re, im;
	unsigned int idx = (startI == 0) ? 0 : (startI * 2);
...
int lime_rrc_interpolate_x2(scmplx *sin, scmplx *sout, int len) {
	...
	pthread_t t[4];
	rrcThreadInfo *threadInfo = (rrcThreadInfo*)malloc(4 * sizeof(rrcThreadInfo));
	
	threadInfo[0].startI = 0;
	threadInfo[0].endI = (len/4) - 1;
	threadInfo[0].sin = sin;
	threadInfo[0].sout = sout;
	threadInfo[1].startI = (len/4);
	threadInfo[1].endI = (len/2) - 1;
	threadInfo[1].sin = sin;
	threadInfo[1].sout = sout;
	threadInfo[2].startI = (len/2);
	threadInfo[2].endI = ((3*len)/4) - 1;
	threadInfo[2].sin = sin;
	threadInfo[2].sout = sout;
	threadInfo[3].startI = ((3*len)/4);
	threadInfo[3].endI = len - 1;
	threadInfo[3].sin = sin;
	threadInfo[3].sout = sout;
	
	for (int i = 0; i < 4; i++) {
		pthread_create(&t[i], NULL, rrcThread, &threadInfo[i]);
	}
	...
	//i
	//[0, (len/4) - 1] -> [0, 4999]
	//[(len/4), (len/2) - 1] -> [5000, 9999]
	//[(len/2), (3*len/4) - 1] -> [10000, 14999]
	//[(3*len/4), len - 1] -> [15000, 19999]
	//For every iteration of i, idx increases by 2
	//idx = i_start * 2
	//[0, 9998)
	//[9998, 19998)
	//[19998, 29998)
...
int main(int argc, const char* argv[])
{
	...
	cout << "UTSC transmitter starting" << endl << "Input file: " << inputFilename << endl << "Channel: " << channel << endl << endl;
